...
const typeNum = (arr) => {
  // Solution code here...
  return (
    arr.filter( obj => typeof(obj) === "number")
  );
};
...
const containsAnd = (arr) => {
  // Solution code here...
  return (arr.filter(obj => obj.includes('and')));
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let outputArr = [];
  arr.map(obj => (obj.baseStat > minBaseStat) ? outputArr.push(obj.stat.name) : false );
  return outputArr;
};
/* ------------------------------------------------------------------------------------------------
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  let arr2 = arr.filter(element => typeof(element) === 'number');
  return arr2.map(number => ((number % 2) === 0) ? 'even' : 'odd');
};
