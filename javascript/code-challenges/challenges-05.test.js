...
const addPurchases = (arr) => {
  // Solution code here...
  return arr.reduce((acc, purchase) => acc + purchase.purchasePrice, 0);
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return arr.reduce((acc, idx) => acc + 1, 0);
};
...
const returnNames = (arr) => {
  // Solution code here...
  let outputArr = [];
  arr.reduce((acc, char) => (outputArr.push(char.name)), []);
  return outputArr;
};
...
const countPrimeNumbers = (arr) => {
  // Solution code here...
  return arr.reduce((acc, num) => isPrime(num)? acc + 1: acc + 0, 0);
};
...
const extractChildren = (arr) => {
  // Solution code here...
  let filteredArr = arr.filter(char => char.name.includes('a'));
  return filteredArr.reduce((acc,char) =>{
    if (char.children){
      char.children.map(kid => acc.push(kid));
    }
    return acc;
  } , []);
};
