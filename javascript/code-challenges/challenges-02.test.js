...
const forLoopTwoToThe = (arr) => {
  ...
  let outputArr = [];
  for (let num in arr) {
    outputArr.push(Math.pow(2, arr[num]));
  }
  return outputArr;
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  return arr.map(stat => stat.ability.name);
};
...
const extractStats = (arr) => {
  ...
  return arr.map(group => (
    {
      'name' : group.stat.name,
      'total' : group.effort + group.baseStat
    }
  ));
};
/* ------------------------------------------------------------------------------------------------
